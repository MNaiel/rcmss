function AllFilrecord=ConverttoTUDreadrecord(recordIn)
NumFrames=length(recordIn);
obj=1;
for i=1:NumFrames
    DW=size(recordIn(i).bb,1);
    for j=1:DW
        BB=recordIn(i).bb(j,:);
        b1=max(min(BB(1),BB(3)),1);
        b2=max(min(BB(2),BB(4)),1);
        b3=max(BB(1),BB(3));
        b4=max(BB(2),BB(4));
        record.objects(j).bbox=[b1,b2,b3,b4];
    end
    record.imgname=recordIn(i).img;
    AllFilrecord.record(i)=record;    
end
return;
notEOF=1;
while (notEOF),
