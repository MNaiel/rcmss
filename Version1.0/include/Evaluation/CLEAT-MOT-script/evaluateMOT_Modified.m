...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [ClearMOT,AllTrk ]= evaluateMOT_Modified(groundtruth,result,dist,dispON,AllTrk)
...
MissedatTimet=zeros(numFrames,1);
FPatTimet=zeros(numFrames,1);
NGTatTimet=zeros(numFrames,1);
DistanceatTimet=zeros(numFrames,1);
Nmapped=zeros(numFrames,1);
MODPatTimet=zeros(numFrames,1);
for i=1:numFrames;
   ...
   if length(mapping)  > 0 &&  length(premapping) > 0
      for o=1:length(mapping(:,1))
         ...
         if mapping(o,2) ~= premapping(idx,2)
            ...
            ID=mapping(o,2);
            AllTrk.Trobject(ID).resultEvaluation(i)=0;
            AllTrk.Trobject(ID).IDSW(i)=1;
         else
            ...
            ID=idxTracks(h);
            AllTrk.Trobject(ID).resultEvaluation(i)=1;
            DistanceatTimet(i)=DistanceatTimet(i)+score(idxo,h);
            Nmapped(i)=Nmapped(i)+1;
         end
   ...
   if Nmapped(i)~=0       
   MODPatTimet(i)=DistanceatTimet(i)/Nmapped(i);
   end
   ...
   for r=1:size(Ass,1)
      if sum(Ass(r,:)) == 0
         ...
         MissedatTimet(i)=MissedatTimet(i)+1;
      end
   end
   if isempty(Ass)
   ...
   for c=1:size(Ass,2)
      if sum(Ass(:,c)) == 0
         ...
         ID=idxTracks(c);
         AllTrk.Trobject(ID).resultEvaluation(i)=0;
         AllTrk.Trobject(ID).IDSW(i)=0;
        FPatTimet(i)=FPatTimet(i)+1;
      end
   end   
...
ClearMOT.MODA = 1. - ((falseneg + falsepos)/gt);
ClearMOT.MODP =sum(MODPatTimet)/ numFrames;
if dispON
   disp('------ ::RESULTS:: ---------');
