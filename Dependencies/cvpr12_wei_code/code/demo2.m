...
%   [centerx,centery,ratioTrue/NormalizedWidth,thetaInitial,trueHeight/trueWidth, ] 
%p=[px, py, sx, sy, theta]';
...
%    affsig(6) = skew angle
p0 = p(4)/p(3);%trueHeight/trueWidth
...
Fi = Fio;    
% Fio: the dictionary for the SGM
% patcho: the patches obtained from the first frame (vector)
...
for f = 1:num
    ...
    paramSR.lambda = 0.01;
    beta = mexLasso(YYY, [AAA_pos AAA_neg], paramSR);  %alpha in the paper, equation(3)
    beta = full(beta);
    ...
    con = exp(-rec_f/gamma)./exp(-rec_b/gamma);             %equation (4)        
    ...
    if f==1                                                         % the template histogram in the first frame and before occlusion handling
        ...
        paramSR.lambda = 0.01;
        alpha_q = mexLasso(xo, Fii, paramSR);                       % equation (5), alpha is beta in paper
